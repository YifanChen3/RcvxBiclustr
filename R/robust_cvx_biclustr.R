...
rcbc <- function(X,tuning_free=c("yes","no"),lambda,k_row=10,k_col=10,tau=NULL,rho=0.35,
                 W1=NULL,W2=NULL,V1=NULL,V2=NULL,Y1=NULL,Y2=NULL,Z1=NULL,Z2=NULL,
                 max_iter=700,tol_abs=1e-05){
  tuning_free <- match.arg(tuning_free)
  if ((tuning_free == "yes") & (is.null(tau) == FALSE)){
    warning("The input tau will not be used since you are choosing tuning-free method.")
  }
  if ((tuning_free == "no") & (is.null(tau) == TRUE)){
    stop("Error: You are choosing the non-tuning-free method while not specifying a tau.")
  }
  n <- as.integer(nrow(X))
  p <- as.integer(ncol(X))
  ...
  X <- X - mean(X[X>quantile(X,.05) & X<quantile(X,.95)])
  X <- X/norm(X,'f')
  scal <- 17 / (quantile(X,.95)-quantile(X,.05))
  X <- X*scal

  wt_row <- robust_weights(X,1.345*mad(X),0.001,k_row)
  wt_col <- robust_weights(t(X),1.345*mad(X),0.001,k_col)
